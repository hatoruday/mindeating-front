...
export default function ClosingHeader({ itemList, satisfiedString, satisfiedExtent }: { itemList: string[]; satisfiedString: string[]; satisfiedExtent: number }) {
      ...
      {isExpanded ? (
        <div className="flex flex-col flex-shrink-0 w-full">
          <div className="flex px-3 w-full justify-between items-center">
            <div className="font-medium text-[16px]">{itemList[0]}</div>
            <div className="flex gap-2 flex-shrink-0 items-center text-[12px] text-[#696972] font-medium">
              {satisfiedString[satisfiedExtent]}
              <SatisfiedImoticon satisfiedExtent={satisfiedExtent} />
            </div>
          </div>
          {itemList.slice(1).map((item, index) => (
            <div key={index} className="flex px-3 w-full justify-between items-center">
              ...
              {index === itemList.length - 2 && (
                <button type="button" onClick={() => setIsExpanded(!isExpanded)} className="flex items-center gap-2">
                  <Image src="/upChevron.svg" width={14} height={8} alt="upChevron" />
                </button>
              )}
            </div>
          ))}
        ...
        <div className="flex  px-3 gap-2 justify-between w-full items-center">
          {itemList.length > 1 ? (
            <button type="button" onClick={() => setIsExpanded(!isExpanded)} className="flex items-center gap-2">
              ...
              <Image src="/downChevron.svg" width={14} height={8} alt="downChevron" />
            </button>
          ) : (
